...
namespace SurfaceCharts
{
    public class ChartStyle2D
    {
        ...
        public ChartStyle2D(SurfaceChartForm fm1)
        {
            form1 = fm1;
            Rectangle rect = form1.ClientRectangle;
            chartArea = new Rectangle(rect.X, rect.Y,
        ...
        public void AddChartStyle2D(Graphics g, ChartStyle cs3d)
        {
            ...
            if (cs3d.IsYGrid == true)
            {
                ...
                for (fX = cs3d.XMin + cs3d.XTick; fX < cs3d.XMax;
                fX += cs3d.XTick)
                {
                    g.DrawLine(aPen, Point2D(new PointF(fX,
                    cs3d.YMin), cs3d),
            ...
            if (cs3d.IsXGrid == true)
            {
                aPen = new Pen(cs3d.GridStyle.LineColor, 1f);
                aPen.DashStyle = cs3d.GridStyle.Pattern;
                for (fY = cs3d.YMin + cs3d.YTick; fY < cs3d.YMax;
                    fY += cs3d.YTick)
                {
            ...
            {
                PointF xAxisPoint = Point2D(new PointF(cs3d.XMin,
                fY), cs3d);
        ...
        private void AddLabels(Graphics g, ChartStyle cs3d)
        {
            ...
            SizeF labelFontSize = g.MeasureString("A",
            cs3d.LabelFont);
            SizeF titleFontSize = g.MeasureString("A",
            cs3d.TitleFont);
            SizeF tickFontSize = g.MeasureString("A",
            cs3d.TickFont);
            SolidBrush aBrush = new SolidBrush(cs3d.TickColor);
            StringFormat sFormat = new StringFormat();
            // Create the x-axis tick marks:
            aBrush = new SolidBrush(cs3d.TickColor);
            for (float fX = cs3d.XMin; fX <= cs3d.XMax;
            fX += cs3d.XTick)
            {
                PointF yAxisPoint = Point2D(new PointF(fX,
                cs3d.YMin), cs3d);
                ...
                g.DrawString(fX.ToString(), cs3d.TickFont, aBrush, new PointF(yAxisPoint.X + sizeXTick.Width / 2 + form1.PlotPanel.Left, yAxisPoint.Y + 4f + form1.PlotPanel.Top), sFormat);
            }
            // Create the y-axis tick marks:
            for (float fY = cs3d.YMin; fY <= cs3d.YMax; fY +=
            cs3d.YTick)
            {
                PointF xAxisPoint = Point2D(new PointF(cs3d.XMin,
                fY), cs3d);
                ...
                form1.PlotPanel.Left,
                xAxisPoint.Y - tickFontSize.Height / 2 +
                form1.PlotPanel.Top), sFormat);
            }
            // Add horizontal axis label:
            aBrush = new SolidBrush(cs3d.LabelColor);
            SizeF stringSize = g.MeasureString(cs3d.XLabel,
            cs3d.LabelFont);
            g.DrawString(cs3d.XLabel, cs3d.LabelFont, aBrush,
            new Point(form1.PlotPanel.Left +
            form1.PlotPanel.Width / 2 -
            ...
            titleFontSize.Height + yOffset / 3 +
            form1.PlotPanel.Height / 2);
            g.RotateTransform(-90);
            g.DrawString(cs3d.YLabel, cs3d.LabelFont,
            aBrush, 0, 0, sFormat);
            // Restore it:
            g.Restore(gState);
            // Add title:
        ...
        public void SetPlotArea(Graphics g, ChartStyle cs3d)
        {
            ...
            cs3d.YMin.ToString(), cs3d.TickFont);
            for (float yTick = cs3d.YMin; yTick <= cs3d.YMax;
            yTick += cs3d.YTick)
            {
            ...
            float topMargin = yOffset +
            titleFontSize.Height + ySpacing;
